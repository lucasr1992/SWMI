...
function CadastroEspecialidade(user: usuario){
  ...
  const findRegister = async() => {
    ...
    setCampos({
      especialidade: resultado.data.especialidade,
      status:'ATIVO',
    })
  }
  ...
  const handleSubmit = useCallback(async (data: requestFiled) => {
    ...
    try{
      ...
      const schema = Yup.object().shape({
        especialidade: Yup.string().required("É Necessario Informar a Especialidade"),
      ...
      try{
        ...
        const response = await api.post('/especialidades', res).then((rest) => {
          const request = rest.data;

         
          localStorage.setItem('@ESPECIALIDADE', data.especialidade.toUpperCase())
          voltar()
        }).catch((error: any) => {
