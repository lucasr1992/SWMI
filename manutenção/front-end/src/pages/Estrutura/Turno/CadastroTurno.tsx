...
function CadastroTurno(){
  ...
  const findRegister = async() => {
    const resultado = await api.get(`/turnos/id/${id}`)
    setCampos({
  ...
  const handleSubmit = useCallback(async (data: requestFiled) => {
    ...
    try{
      ...
      const schema = Yup.object().shape({
        turno: Yup.string().required("É Necessario Informar o Turno"),
      ...
      try{
        ...
        const response = await api.post('/turnos', res).then((rest) => {
          const request = rest.data;

         
          localStorage.setItem('@TURNO', data.turno.toUpperCase())
          voltar()
        }).catch((error: any) => {
