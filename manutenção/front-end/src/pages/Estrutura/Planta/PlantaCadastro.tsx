...
function CadastroPlanta(user:usuario){
  ...
  async function loadPlantaAtiva(){
    try{
      const list = (await api.get('/unidades/status/ATIVO')).data
  ...
  const salvar = useCallback(async (data: requestFileds) => {
    ...
    try{
      ...
      const schema = Yup.object().shape({
        planta: Yup.string().required("É Necessario Informar a Planta!"),
        regiao:   Yup.string().required("Necessario Informar a Região"),
        pais:   Yup.string().required("Necessario Informar o Pais"),
        unidade:   Yup.number().required("Necessario Informar a Planta"),
      });
      ...
      const registro = JSON.parse(`{"planta": "${data.planta}","regiao": "${data.regiao}", "pais": "${data.pais}" ,"unidade": { "id": ${data.unidade} }, "status": "ATIVO"}`)
      ...
      const response = await api.post('/plantas', res).then((rest) => {
        const request = rest.data;
        
        localStorage.setItem('@PLANTA', data.planta.toUpperCase())
          voltar()
      }).catch((error: any) => {
    ...
    <Content>
      ...
      <Form   ref={formRef} onSubmit={salvar}>
        ...
        <div className="regiao">
          <label>Região</label>
          <InputDefalt  name={"regiao"} Main={true} id='centrocusto' />
        </div>
        <div className="pais">
          <label>Pais</label>
          <InputDefalt  name={"pais"} Main={true} id='centrocusto' />
        </div>
        <div className="unidade">
          <label>Unidade</label>
          <SelectDefault  options={selectValue} name={'unidade'} Main={true}/>
        </div>
